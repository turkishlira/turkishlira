...
#if defined(NDEBUG)
# error "Turkishlira cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x8fcd98b72a01ce69988bcb2706d4eb0bdd3023f2dc721f4a639d2b8ec1efd1ef");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Turkishlira: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Turkishlira Signed Message:\n";
...
{
    // Turkishlira: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Turkishlira: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Turkishlira: 3.5 days
static const int64 nTargetSpacing = 3 * 60; // Turkishlira: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Turkishlira: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Turkishlira: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1aebaa286f05a68ce2fb46619eca1b29f59f019da6d7f520903ea7c47d3602bb");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Turkish founding father Mustafa Kemal Atatürk was born in 1881 in Salonika";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("043ea1f047a53ec7bdf25446632d7e5f2466365472cf66c7aaa7a091444bccdde088034943f5cb303008027b86ceb9912c2af998e4a8359cb9564138ce660cbde8") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1540728241;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087328226;

        if (fTestNet)
        {
            block.nTime    = 1540728210;
            block.nNonce   = 389617058;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xb758aa5182edd4e6250f5f4323393daf9d86513792d94dff91b3af92bbdddc25"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf4, 0xca, 0xa1, 0xd0 }; // Turkishlira: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TurkishliraMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TurkishliraMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TurkishliraMiner(CWallet *pwallet)
{
    printf("TurkishliraMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("turkishlira-miner");
    ...
    try { loop {
        ...
        printf("Running TurkishliraMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TurkishliraMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TurkishliraMiner, pwallet));
}
